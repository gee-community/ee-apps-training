...
var images = ee.ImageCollection('COPERNICUS/S2_HARMONIZED').filterDate(start, end)
...
Map.addLayer(images.select(vis.bands).reduce(ee.Reducer.percentile([25])).rename(vis.bands), vis, 'images', false)
...
Map.onClick(function(pt) {
  ...
  var imagesLocal = images.filterBounds(pt).map(function(i) {
  ...
  // inspect images
  timelapse.inspect(imagesLocal, pt, vis)
})
